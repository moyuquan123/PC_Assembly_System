...
面向装机新手的完整 MVP：按类别与品牌浏览单个配件、查看官方与用户投稿的完整配置方案、分八步调整配件、即时检查兼容性、创建社区账号、上传配置、评论与推荐，并保存本地草稿和生成不可枚举的分享链接。
...
- `apps/api`：Fastify 5 API、普通用户与管理员双会话认证、社区互动、混合排序、限流、服务端兼容复核
