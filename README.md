...
面向装机新手的完整 MVP：按类别与品牌浏览单个配件、查看官方与用户投稿的完整配置方案、分八步调整配件、即时检查兼容性、上传自己的配置、保存本地草稿并生成不可枚举的分享链接。

配置投稿无需普通用户账号，但必须包含完整八类配件，并由服务端重新检查在售状态和兼容性。确定性智能推荐引擎与 API 仍作为后续 AI 需求解析的安全基础保留，不依赖外部 AI 密钥。
