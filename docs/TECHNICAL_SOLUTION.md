...
本项目采用 TypeScript 全栈、前后端分离的模块化单体架构。在保留现有 React + Vite 原型的基础上，新增 Fastify API、PostgreSQL 数据库和对象存储，不在 MVP 阶段引入微服务、消息队列或独立搜索集群。
...
- 若后续增加手机号、邮箱、支付、AI 推荐或第三方电商跳转，重新进行隐私和合规评审。
