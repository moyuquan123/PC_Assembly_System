import { CheckCircle2, Search, SlidersHorizontal, TriangleAlert, Upload } from "lucide-react";
...
import { getPublishedConfigurations } from "../lib/api";
...
export default function ConfigurationLibraryScreen({ parts, draft, loading, error, onApply, onStart, onPublish }: {
  parts: Part[];
  draft: BuildDraft;
  loading: boolean;
  error: string;
  ...
  }), [configurationClass, configurations, deferredQuery, source]);
  ...
  return <main className="configuration-library-screen">
    <header className="configuration-library-heading configuration-plan-heading"><div><h1>配置方案</h1><p>浏览完整装机方案，也可以分享你自己的配置。</p></div><div><button className="primary-button" type="button" onClick={() => setUploadOpen(true)}><Upload size={18} />上传我的配置</button><button className="secondary-button" type="button" onClick={onStart}>开始装机</button></div></header>
    ...
    <section className="configuration-results" aria-labelledby="configuration-results-title">
      <div className="configuration-results-heading"><h2 id="configuration-results-title">共 {visibleConfigurations.length} 套方案</h2>{source !== "全部方案" || configurationClass !== "全部" || query ? <button type="button" onClick={() => { setQuery(""); setSource("全部方案"); setConfigurationClass("全部"); }}>清除筛选</button> : null}</div>
      {communityQuery.error ? <p className="community-load-note">用户投稿暂时无法加载，官方方案仍可正常浏览。</p> : null}
      {visibleConfigurations.length > 0 ? <div className="configuration-table-wrap"><table className="configuration-table plan-table">
        <thead><tr><th>配置名称</th><th>来源</th><th>配置分类</th><th>CPU</th><th>显卡</th><th>参考总价</th><th>兼容性状态</th><th>操作</th></tr></thead>
        <tbody>{visibleConfigurations.map((configuration) => <ConfigurationRow key={`${configuration.source}-${configuration.id}`} configuration={configuration} onOpen={setDetail} onApply={onApply} />)}</tbody>
    ...
    <UploadConfigurationModal open={uploadOpen} draft={draft} parts={parts} onClose={() => setUploadOpen(false)} onGoToBuilder={() => { setUploadOpen(false); onStart(); }} onSubmit={submit} />
  </main>;
}
...
function ConfigurationRow({ configuration, onOpen, onApply }: { configuration: MarketConfiguration; onOpen: (configuration: MarketConfiguration) => void; onApply: (configuration: MarketConfiguration) => void }) {
  ...
  return <tr>
    <td data-label="配置名称"><strong className="configuration-name">{configuration.name}</strong>{configuration.description ? <small className="configuration-description">{configuration.description}</small> : null}</td>
